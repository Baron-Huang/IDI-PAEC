...
class Read_Feats_Datasets(Dataset):
    def __init__(self, data_path = None, label_path = None, label_size = [1, 1]):
    ...
    def __getitem__(self, item):
        ...
        if data_inter.ndim == 3:
            data_inter_tensor = torch.tensor(data_inter).reshape(data_inter.shape[1], data_inter.shape[2])
        else:
            data_inter_tensor = torch.tensor(data_inter)
