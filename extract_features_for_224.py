...
def other_paras_cfg():
    ...
    paras.add_argument('--pretrained_weights_path', type=str,
        default='/home/dataset-hpfs-0/Kevin_Huang/IGI_PAEC_public/Pretrained_wegts/Swin_trans/swin_tiny_patch4_window7_224_22k.pth')
...
class SwinT_feats_encoder(nn.Module):
    def __init__(self, base_model=None, final_avp_kernel_size=base_paras_cfg().window_size ** 2,
                 final_avp_tride=base_paras_cfg().window_size ** 2):
...
def extract_features_for_swint(base_args=None, other_args=None, save_path=None, read_path = None,
    ...
    feats_extract_model = feats_extract_model.cuda(gpu_device)
    with torch.no_grad():
        count_save = 0
        input_len = 100
        ...
        for img_data_list, img_label in train_loader:
            count_save += 1
            pre_y = torch.zeros((1, 768)).cuda(gpu_device)
            for img_data in img_data_list:
                if img_data.shape[0] <= input_len:
                    pre_y = torch.cat((pre_y, feats_extract_model(img_data.cuda(gpu_device))))
                else:
                    xxx_count = int(img_data.shape[0] / input_len)
                    for img_data_i in range(xxx_count):
                        kkk = feats_extract_model(
                            img_data[input_len*img_data_i:(input_len*(img_data_i+1)), :, :, :].cuda(gpu_device))
                        pre_y = torch.cat((pre_y, kkk))

                    if xxx_count * input_len < img_data.shape[0]:
                        k = feats_extract_model(img_data[xxx_count * input_len:, :, :, :].cuda(gpu_device))
...
if __name__ == '__main__':
    new_tensor = extract_features_for_swint(base_args=base_paras_cfg(), other_args=other_paras_cfg(),
            save_path='/home/dataset-hpfs-0/Kevin_Huang/IGI_PAEC_public/Datasets/AMU_LSCC/LSCC_pretrained_without_PE/Test',
            read_path=r'/home/dataset-hpfs-0/Kevin_Huang/IGI_PAEC_public/Datasets/AMU_LSCC/LSCC_WSI_without_PE/Test',
            gpu_device = 3)
