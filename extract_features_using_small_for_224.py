...
def other_paras_cfg():
    ...
    paras.add_argument('--bags_len', type=int, default=5000)
    ...
    paras.add_argument('--pretrained_weights_path', type=str,
     default='/home/dataset-hpfs-0/Kevin_Huang/IGI_PAEC_public/Pretrained_wegts/Swin_trans/swin_small_patch4_window7_224_22k.pth')
...
def base_paras_cfg():
    ...
    paras.add_argument('--num_classes', type=int, default=5)
    paras.add_argument('--depths', type=list, default=[2, 2, 18, 2])
    ...
    paras.add_argument('--window_size', type=int, default=7)

    return paras.parse_args()

class SwinT_feats_encoder(nn.Module):
    def __init__(self, base_model=None, final_avp_kernel_size=49, final_avp_tride=49):
...
def extract_features_for_swint(base_args=None, other_args=None, save_path=None, read_path = None):
    ...
    with torch.no_grad():
        ...
        summary(feats_extract_model, (3, 224, 224), device='cpu')
        print(feats_extract_model(torch.randn((3, 3, 224, 224), device='cpu')).shape)
    #print(feats_extract_model)

    feats_extract_model = feats_extract_model.cuda(3)
    with torch.no_grad():
        count_save = 0
        input_len = 100
        ...
        for img_data_list, img_label in train_loader:
            count_save += 1
            pre_y = torch.zeros((1, 768)).cuda(3)
            for img_data in img_data_list:
                if img_data.shape[0] <= input_len:
                    pre_y = torch.cat((pre_y, feats_extract_model(img_data.cuda(3))))
                else:
                    xxx_count = int(img_data.shape[0] / input_len)
                    for img_data_i in range(xxx_count):
                        kkk = feats_extract_model(img_data[input_len*img_data_i:(input_len*(img_data_i+1)), :, :, :].cuda(3))
                        pre_y = torch.cat((pre_y, kkk))

                    if xxx_count * input_len < img_data.shape[0]:
                        k = feats_extract_model(img_data[xxx_count * input_len:, :, :, :].cuda(3))
                        pre_y = torch.cat((pre_y, k))
                    else:
                        pass

            pre_y = pre_y[1:]
            swint_feats_mat = pre_y.detach().cpu().numpy()
            label = img_label.detach().cpu().numpy()
            try:
                os.makedirs(save_path + '/Feats')
...
if __name__ == '__main__':
    new_tensor = extract_features_for_swint(base_args=base_paras_cfg(), other_args=other_paras_cfg(),
        save_path='/home/dataset-hpfs-0/Kevin_Huang/IGI_PAEC_public/Datasets/DHMC_Kidney/Kidney_pretrained_without_PE/Test',
        read_path=r'/home/dataset-hpfs-0/Kevin_Huang/IGI_PAEC_public/Datasets/DHMC_Kidney/Kidney_WSI_without_PE/Test')
