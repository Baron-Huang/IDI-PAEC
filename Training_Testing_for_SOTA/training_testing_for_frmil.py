...
def view_results_for_frmil(mil_net=None, train_loader=None, data_parallel=False, loss_fn=None, proba_mode=False,
        ...
        if data_parallel == False:
            ...
            with torch.no_grad():
                ...
                for train_img in train_img_list:
                    if bags_stat == True:
                        _, sti_pre_y = mil_net(train_img.cuda(gpu_device))
                    else:
...
def training_for_frmil(mil_net=None, train_loader=None, val_loader=None, test_loader=None, proba_mode=False,
    ...
    for i in range(epoch):
        ...
        if lr_fn == 'vit_public':
            ...
            assert print('The learning rate is erorr!!!!')
        ...
        for img_data_list, img_label in train_loader:
            if data_parallel == False:
                ...
                for img_data in img_data_list:
                    if bags_stat == True:
                        _, mil_pre_y = mil_net(img_data.cuda(gpu_device))
                    else:
        ...
        train_acc, train_loss, _, _ = view_results_for_frmil(mil_net=mil_net, train_loader=train_loader, data_parallel=data_parallel,
        ...
        val_acc, val_loss, _, _ = view_results_for_frmil(mil_net=mil_net, train_loader=val_loader, data_parallel=data_parallel,
                                                    loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                    proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)
    ...
    test_acc, test_loss, _, _ = view_results_for_frmil(mil_net=mil_net, train_loader=test_loader, data_parallel=False,
                                                 loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                 proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)
...
def testing_for_frmil(test_model=None, train_loader=None, val_loader=None, test_loader=None, gpu_device=0,
                      out_mode=None, proba_mode=False, proba_value=0.5, class_num = 2,
                      roc_save_path = None, bags_stat = True, bag_relations_path = None,
                      resu_text_path = None):
    loss_fn = nn.CrossEntropyLoss()
    train_acc, train_loss, _, _ = view_results_for_frmil(mil_net=test_model, train_loader=train_loader, data_parallel=False,
                                                          loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                          proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)

    test_acc, test_loss, pre_y_sum, label_sum = view_results_for_frmil(mil_net=test_model, train_loader=test_loader,
