...
def view_results_for_rrtmil(mil_net=None, train_loader=None, data_parallel=False, loss_fn=None, proba_mode=False,
...
def training_for_rrtmil(mil_net=None, train_loader=None, val_loader=None, test_loader=None, proba_mode=False,
    ...
    for i in range(epoch):
        ...
        for img_data_list, img_label in train_loader:
            if data_parallel == False:
                ...
                for img_data in img_data_list:
                    if bags_stat == True:
                        mil_pre_y = mil_net(img_data.cuda(gpu_device), pre_y)
                    else:
                        mil_pre_y = mil_net(img_data.cuda(gpu_device))
                    pre_y = torch.cat((pre_y, mil_pre_y))
            pre_y = pre_y[1:]
        ...
        train_acc, train_loss, _, _ = view_results_for_rrtmil(mil_net=mil_net, train_loader=train_loader,
        ...
        val_acc, val_loss, _, _ = view_results_for_rrtmil(mil_net=mil_net, train_loader=val_loader,
                                                            data_parallel=data_parallel,
                                                    loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                    proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)
    ...
    test_acc, test_loss, _, _ = view_results_for_rrtmil(mil_net=mil_net, train_loader=test_loader, data_parallel=False,
                                                 loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                 proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)
...
def testing_for_rrtmil(test_model=None, train_loader=None, val_loader=None, test_loader=None, gpu_device=0,
                       out_mode=None, proba_mode=False, proba_value=0.5, class_num = 2,
                       roc_save_path = None, bags_stat = True, bag_relations_path = None,
                       resu_text_path = None):
    loss_fn = nn.CrossEntropyLoss()
    train_acc, train_loss, _, _ = view_results_for_rrtmil(mil_net=test_model, train_loader=train_loader, data_parallel=False,
                                                          loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                          proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)


    test_acc, test_loss, pre_y_sum, label_sum = view_results_for_rrtmil(mil_net=test_model, train_loader=test_loader,
