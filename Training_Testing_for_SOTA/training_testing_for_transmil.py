...
def view_results_for_transmil(mil_net=None, train_loader=None, data_parallel=False, loss_fn=None, proba_mode=False,
...
def training_for_transmil(mil_net=None, train_loader=None, val_loader=None, test_loader=None, proba_mode=False,
                        ...
                        proba_value=0.85, class_num = 2, bags_stat = True, resu_text_path = None):
    ...
    for i in range(epoch):
        ...
        if lr_fn == 'vit_public':
            rmp_optim = torch.optim.AdamW(mil_net.parameters(), lr=vit_public_lr_schedule(i))
        elif lr_fn == 'vit_amu':
            rmp_optim = torch.optim.AdamW(mil_net.parameters(), lr=vit_amu_lr_schedule(i))
        elif lr_fn == 'cnn':
            pass
        else:
            print('erorr!!!!')
        ...
        train_acc, train_loss, _, _ = view_results_for_transmil(mil_net=mil_net, train_loader=train_loader,
        ...
        val_acc, val_loss, _, _ = view_results_for_transmil(mil_net=mil_net, train_loader=val_loader,
                                                            data_parallel=data_parallel,
                                                    loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                    proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)
    ...
    test_acc, test_loss, _, _ = view_results_for_transmil(mil_net=mil_net, train_loader=test_loader, data_parallel=False,
                                                 loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                 proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)
...
def testing_for_transmil(test_model=None, train_loader=None, val_loader=None, test_loader=None, gpu_device=0,
                       out_mode=None, proba_mode=False, proba_value=0.5, class_num = 2, roc_save_path = None,
                       bags_stat = True, bag_relations_path = None, resu_text_path = None):

    loss_fn = nn.CrossEntropyLoss()
    train_acc, train_loss, _, _ = view_results_for_transmil(mil_net=test_model, train_loader=train_loader, data_parallel=False,
                                                          loss_fn=loss_fn, proba_mode=proba_mode, gpu_device=gpu_device,
                                                          proba_value=proba_value, class_num=class_num, bags_stat=bags_stat)

    test_acc, test_loss, pre_y_sum, label_sum = view_results_for_transmil(mil_net=test_model, train_loader=test_loader,
    ...
    print('train_acc:{:.4}'.format(np.mean(train_acc)),
          ' test_acc:{:.4}'.format(np.mean(test_acc)), '\n')
    print('train_acc:{:.4}'.format(np.mean(train_loss)),
          ' test_acc:{:.4}'.format(np.mean(test_loss)), '\n')

    print('########################## testing set results #########################')
    k = classification_report(label_sum, pre_label_sum, digits=4)
    print(k)

    print('########################## auc results #########################')
    t = roc_auc_score(np.reshape(label_proba_sum, (label_proba_sum.shape[0] * label_proba_sum.shape[1])),
...
def training_for_transmil_kidney_lung_bcnc(mil_net=None, train_loader=None, val_loader=None, test_loader=None, proba_mode=False,
                        lr_fn=None, epoch=100, gpu_device=0, onecycle_mr=1e-2, current_lr=None, data_parallel=False,
                        weight_path=r'E:\SOTA_Model_Interpretable_Learning\SIL_Weights\Larynx\SwinT_1_.pth',
                        proba_value=0.85, class_num = 2, bags_stat = True, resu_text_path = None):
    loss_fn = nn.CrossEntropyLoss()
    ...
    for i in range(epoch):
        start_time = time.time()
        if lr_fn == 'vit_public':
            rmp_optim = torch.optim.AdamW(mil_net, lr=vit_public_lr_schedule(i))
        elif lr_fn == 'vit_amu':
            rmp_optim = torch.optim.AdamW(mil_net, lr=vit_amu_lr_schedule(i))
        elif lr_fn == 'cnn':
            pass
        else:
            print('erorr!!!!')
