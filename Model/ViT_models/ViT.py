'''
###Funcition:ViT of manuscript:'Transformer-CNN Neural Network of Depth Domain
...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 14, 'input_size': (3, 224, 224), 'pool_size': None,
...
class Block(nn.Module):
    '''单个的VisionTransformer模块,由很多个这个ViT模块堆成了最后的完整的ViT'''
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        #B, C, H, W = x.shape
        #print(x.shape)         #[70, 3, 224, 224]
        # FIXME look at relaxing size constraints
        # assert H == self.img_size[0] and W == self.img_size[1], \
        #     f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x).flatten(2).transpose(1, 2)
        #print(x.shape)         #[70, 196, 768]
        return x


class HybridEmbed(nn.Module):
    """ CNN Feature Map Embedding，从CNN中提取特征图，进行特征图编码
...
class VisionTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=4, embed_dim=768, depth=12,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))      #用来对patches进行位置编码使用
        ...
        self.apply(self._init_weights)      #只是用来加载预训练权重用的
    ...
    def forward_features(self, x):
        ...
        x = x + self.pos_embed      #在进行位置编码
        x = self.pos_drop(x)        #Dropout操作
...
def _create_vision_transformer(variant, pretrained=False, distilled=False, **kwargs):
    ...
    default_num_classes = default_cfg['num_classes']    #4
    default_img_size = default_cfg['input_size'][-1]    #224

    num_classes = kwargs.pop('num_classes', default_num_classes)    #4
    img_size = kwargs.pop('img_size', default_img_size)             #224
...
@register_model
def vit_base_resnet50_224(pretrained=False, **kwargs):
    """ Custom ViT base hybrid w/ ResNet50D stride 32. No pretrained weights.
    """
    backbone = resnet50(pretrained=pretrained, in_chans=kwargs.get('in_chans', 3), features_only=True, out_indices=[4])
    model_kwargs = dict(embed_dim=768, depth=12, num_heads=12, hybrid_backbone=backbone, **kwargs)
